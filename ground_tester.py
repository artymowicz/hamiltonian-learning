...
if __name__ == '__main__':
	...
	### initialize state
	state = EquilibriumState(params['n'],H,H_name,np.inf)
	...
	expectations_evaluator = lambda x : threebody_expectations_dict[x]
	hamiltonian_learned_coefficients, C, F = hamiltonian_learning.learnHamiltonianFromGroundstate(params['n'], onebody_operators, hamiltonian_terms, expectations_evaluator, params, state.metrics)
	H_learned = Hamiltonian(params['n'], hamiltonian_terms, hamiltonian_learned_coefficients)

	### save results
	saveLearningResults(H, H_name, 0, H_learned, 0, threebody_expectations_dict, params, metrics)
