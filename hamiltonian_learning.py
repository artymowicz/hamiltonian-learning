...
def learnHamiltonianFromThermalStateOld(n, onebody_operators, hamiltonian_terms, expectations_evaluator, params_dict, metrics, return_extras = False):
	assert hamiltonian_terms[0]=='I'*n
	...
	W = W.toarray()
	utils.tprint('plotting')
	utils.plotSpec(W, print_lowest = 10, yscale = 'log')

	W_eigvals, W_eigvecs = scipy.linalg.eigh(W)
	first_eigvec = np.real(W_eigvecs[:,1])
	second_eigvec = np.real(W_eigvecs[:,2])
	#rescale:
	first_eigvec_normalization = first_eigvec[1:]@hamiltonian_terms_expectations[1:]
	first_eigvec = -first_eigvec/first_eigvec_normalization

	second_eigvec_normalization = second_eigvec[1:]@hamiltonian_terms_expectations[1:]
	second_eigvec = -second_eigvec/second_eigvec_normalization

	sorted_indices = sorted(list(range(len(hamiltonian_terms))), key = lambda i: utils.compressPauliToList(hamiltonian_terms[i]))
	for i in sorted_indices:
		if max(np.abs(first_eigvec[i]),np.abs(second_eigvec[i])) > 1e-10:
			print(f'{hamiltonian_terms[i]}   {first_eigvec[i]:.10f}  {second_eigvec[i]:.10f}')

	second_eigvec_normalization = second_eigvec[1:]@hamiltonian_terms_expectations[1:]
	second_eigvec = -second_eigvec/second_eigvec_normalization
	second_eigvec[0] = 1

	#return second_eigvec, 1, {}
	...
	constraints += [T*logDelta + np.conjugate(E.T)@cp.reshape(F_vectorized@X, (r,r))@E + np.eye(r-cutoff)*mu >> 0]
	...
	prob = cp.Problem(cp.Minimize(objective), constraints)
	prob.solve(canon_backend=cp.SCIPY_CANON_BACKEND, solver = params_dict['solver'], verbose = params_dict['printing'])#, save_file = 'dump.ptf')
...
def learnHamiltonianFromThermalStateNoisy(n, onebody_operators, hamiltonian_terms, expectations_evaluator, params_dict, metrics, return_extras = False):
	assert hamiltonian_terms[0]=='I'*n
	...
	if params_dict['objective'] == 'l2':
		...
		raise ValueError(f"objective {params_dict['objective']} not recognized. Valid inputs are 'mu','l1', 'l2, 'rand_linear', 'T', or 'minus_T'")
	...
	if params_dict['objective'] == 'mu':
		...
		constraints += [T*logDelta + np.conjugate(E.T)@cp.reshape(F_vectorized@X, (r,r))@E + np.eye(r-cutoff)*params_dict['mu'] >> 0]
